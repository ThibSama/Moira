...
History writes run on a `HistoryCoordinator` daemon worker thread that models capacity as one in-flight batch plus one pending batch. An in-flight batch with an empty pending slot accepts a new batch without saturation. Only when the pending slot is occupied does the newest batch replace it and latch `backlog saturated`. The saturation latch is separate from the write diagnostic: public status stays `backlog saturated` even when the retained write fails. Only successful completion of the retained or a later generation clears saturation. The coordinator's constructor validates `db_timeout < shutdown_timeout`; production uses `db_timeout=1.0`, `shutdown_timeout=3.0`. The worker wakes on `Condition.notify_all()` from enqueue and shutdown, not periodic polling. Shutdown is idempotent, rejects new work, and `_thread` is never falsely cleared while alive. History failure returns only a sanitized diagnostic and does not disrupt quota state, display, or alerts.
