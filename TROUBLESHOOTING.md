...
History writes run on a `HistoryCoordinator` daemon worker thread that models capacity as one in-flight batch plus one pending batch. An in-flight batch with an empty pending slot accepts a new batch without saturation. Only when the pending slot is occupied does the newest batch replace it and latch `backlog saturated`. The saturation latch is separate from the write diagnostic: public status stays `backlog saturated` even when the retained write fails. Only successful completion of the retained or a later generation clears saturation. A successful write fires `set_write_success_callback` which schedules a History tab refresh via `GLib.idle_add`. Failed, dropped, or saturated writes do not trigger a refresh. The `HistoryReader` is genuinely bounded: at most one running read and one pending newest request. Rapid changes replace only the pending request, never submit unbounded work. Stale generations never publish. Results are dispatched via `GLib.idle_add` so no callback updates a destroyed page. Absent databases are detected without creation. Schema mismatches return `schema mismatch`, not `database unavailable`. View models use tuples for deep immutability. The coordinator's constructor validates `0 < db_timeout < shutdown_timeout`; defaults are `db_timeout=1.0`, `shutdown_timeout=3.0`. `shutdown(timeout=...)` validates the effective timeout before mutating lifecycle state. Lifecycle states: NEW, RUNNING, SHUTTING_DOWN, TERMINATED. Shutdown-before-start terminates immediately and rejects future work. Start-after-terminal is a no-op. The worker wakes on `Condition.notify_all()`, not periodic polling. Shutdown is idempotent, rejects new work, and `_thread` is never falsely cleared while alive. History failure returns only a sanitized diagnostic and does not disrupt quota state, display, or alerts.
