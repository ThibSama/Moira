...
History writes run on a bounded daemon worker thread via a queue with `maxsize=1`. Refresh rendering and alerts never await history I/O. If the queue is full, the new batch is silently dropped — the next refresh will enqueue again. History failure (database locked, disk error, schema mismatch) returns only a sanitized diagnostic string and does not disrupt quota state, display, or alerts. Missing token telemetry does not affect quota collection, display, alerts, or quota-history recording.
