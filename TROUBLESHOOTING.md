...
History writes run on a `HistoryCoordinator` daemon worker thread with a newest-wins overflow policy, race-free state under a single `Condition`, and monotonically increasing generation tracking. Refresh rendering and alerts never await history I/O. If the worker is in-flight or the pending slot is occupied, the newest pending batch replaces any older pending batch, and a sanitized `backlog saturated` status is latched — cleared only after the retained newest generation has completed successfully. An older in-flight success cannot clear saturation latched by a newer enqueue. The coordinator exposes an idempotent `shutdown` connected to the window `close-request` signal. Shutdown rejects new work, signals the worker via the condition, and joins with a bounded timeout (2 seconds) parameterized below the SQLite write timeout. `_thread` is never set to None while the captured thread is still alive. History failure returns only a sanitized diagnostic string and does not disrupt quota state, display, or alerts. `SchemaVersionError` distinguishes schema-version failures from domain validation failures; `write_history_safely` returns exactly `ok`, `database unavailable`, `schema mismatch`, `invalid observation`, or `backlog saturated`.
