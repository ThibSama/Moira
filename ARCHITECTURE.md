...
History writes are integrated into refresh completion (`_record_history`) via a bounded worker queue (`maxsize=1`) running on a daemon thread. Refresh rendering and alerts never await history I/O. If the queue is full, the new batch is silently dropped — the next refresh will enqueue again. `write_history_safely` returns only a bounded `HistoryWriteResult` with a sanitized diagnostic string (`ok`, `database unavailable`, `schema mismatch`, `invalid observation`). History failure does not disrupt quota state, display, or alerts. Missing token telemetry does not affect quota collection, display, alerts, or quota-history recording. No raw payloads, prompts, responses, transcript text, private paths, account identifiers, or secrets are stored.
