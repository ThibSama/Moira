...
**Edit providers** manages local provider profiles and their credentials. Profiles are immutable and provider-neutral: stable slug, display label, closed kind (`deepseek`, `openai_compatible`, `openrouter`, `anthropic`, `openai`, `local`, `custom`), model identifier, enabled flag and an optional sanitized API base URL/Hermes label — persisted additively in config v4 with exact keys, strict booleans and deterministic ordering (every v3 setting is preserved; invalid v4 data falls back without partial acceptance). Per-profile API keys live in GNOME Keyring keyed by slug and purpose (the NTFY token keeps its own item); secrets and fingerprints never enter JSON, History, activity, diagnostics, exports or logs. Base-URL policy: remote kinds require HTTPS; `local` allows only loopback addresses; embedded credentials, query strings, fragments, control characters and oversized values are rejected. Profile changes are local only — Moira never writes Hermes configuration, hooks or external files. Blank credential input preserves the existing secret; only an explicit Remove clears it. Removing a profile clears its Moira-owned Keyring credential only after confirmation; changing a slug is create-new plus explicit old removal. Keyring and persistence run off the GTK thread through bounded generations (stale completions never overwrite newer form state); the UI shows only `configured` / `not configured` and translated sanitized outcomes, never raw exceptions, URLs or profile JSON.
