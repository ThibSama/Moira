...
The inventory is read-only in this release: the probe first checks `hermes --version` and `hermes --help`, then reads only the bounded JSON of `hermes config get model --json` (the `default` and `provider` fields) and `hermes config get providers --json` (provider slugs and scalar default models). It never parses the whole Hermes YAML, never reads `.env` or `auth.json`, never exposes base URLs, credentials, paths or raw output, and never invokes inference. Each provider row shows independent capability badges — activity, quota percentage, exact tokens, balance and cost — with exact states: Claude stays percentage-only, Codex keeps its accepted exact account-usage contract, and unknown cost/balance is shown as a state (never zero). DeepSeek balance is reported as not configured: no `/user/balance` call is made. Provider editing, Keyring credentials, DeepSeek balance and financial units are deferred to a later release.
