...
Profile/credential writes are RECOVERABLE transactions across the config file and the Keyring. Before any side effect, a minimal atomic `0600` journal is written under `$XDG_STATE_HOME/moira` recording the operation kind, validated slugs, the phase (`staged` → `staged-secret` → `config-committed`) and sanitized profile metadata only — never credentials, fingerprints or raw exceptions. The journal is validated EXACTLY on read: the key set must be precisely the eight documented fields, every field is type-checked before any default (non-bool integer version, string slugs, real boolean backup flag), and the phase invariants hold (a save carries a profile; `old_slug` never equals the profile slug; a non-empty `secret_slug` always matches the profile slug; a backup implies a staged secret; a removal never carries a profile, secret or backup field) — valid 7f/7g journals keep parsing. Overwritten credentials are first backed up to a Moira-owned `backup` Keyring entry, so no credential is ever lost — target lookups are strict: FOUND gets a durable backup before overwrite, ABSENT gets none, and UNAVAILABLE fails before any journal or mutation (an unknown credential is never overwritten). On the next editor reload (and, off the GTK thread, at app start through a dedicated single-worker bootstrap executor that disables every config/profile mutation control until it lands), any journaled operation is recovered idempotently to one documented consistent state: `staged` rolls back nothing, `staged-secret` rolls the secret effect back (restoring the backup), and `config-committed` completes the operation forward (upserting the profile, clearing the renamed slug's old credential, dropping the backup). A known config failure durably transitions the journal BEFORE touching anything: with a staged secret the journal moves to the rollback phase, otherwise (metadata-only edits, toggles, removals) to the no-op `staged` phase — a failed cleanup can never leave a forward journal that would complete an already-failed op, and retry recovery preserves the original config, profile and credential. A rename-tail failure restores the config first and only then transitions; if the restoration itself fails, forward recovery is retained. Backup cleanup after a committed overwrite is MANDATORY: if it fails, the forward journal is kept, a sanitized failure is returned and idempotent recovery removes the backup before clearing the journal — success is never reported while a Moira backup remains, and never while any required journal remains. A failed reload recovery executes no pending mutation (the parked work is discarded; only a later successful reload admits mutations). Keyring lookups are typed (FOUND / ABSENT / UNAVAILABLE): an unavailable vault is never treated as absence, so a blank-credential rename fails closed instead of clearing the last credential. All config mutations go through one lock-protected read-merge-write boundary, so concurrent Notifications saves and profile writes preserve each other's changes. Pending mutations are typed deltas applied to the latest committed collection, so rapid edits cannot overwrite each other; a rejected submit clears its slot, shows a fixed translated outcome and accepts later work. After shutdown, no new or queued-not-started operation writes; an operation already past its durable commit boundary may finish consistently but publishes no callback. Keyring and persistence run off the GTK thread through bounded generations (stale completions never overwrite newer form state); the editor is transient to the main window, closes with bounded shutdown; the UI shows only `configured` / `not configured` and translated sanitized outcomes, never raw exceptions, URLs or profile JSON.
